...
const IMAGE_CACHE_VERSION_KEY = 'balatro_img_cache_ver';
...
function getImageCache() {
    try {
        const storedVersion = localStorage.getItem(IMAGE_CACHE_VERSION_KEY);
        if (storedVersion !== String(IMAGE_CACHE_VERSION)) {
            localStorage.removeItem(IMAGE_CACHE_KEY);
            localStorage.setItem(IMAGE_CACHE_VERSION_KEY, String(IMAGE_CACHE_VERSION));
            return {};
        }
...
function tryUrlChain(imgElement, urls, index, category, id) {
    if (index >= urls.length) {
        ...
        const card = imgElement.closest('.item-card');
    ...
    imgElement.onload = function onLoad() {
        ...
        imgElement.classList.add('loaded');
        const card = imgElement.closest('.item-card');
        if (card) card.classList.remove('image-error');
    };
